...
const AudioEditor = () => {
  ...
  const fetchAudioFiles = useCallback(async () => {
    try {
      ...
      const response = await axios.get('http://localhost:3000/api/audio/files', {
        headers: {
  ...
  const fetchBlobAndRenderWaveform = useCallback(async (file, wsInstance) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`http://localhost:3000/api/audio/files/${file._id}/stream`, {
        responseType: 'blob',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleDownloadActiveFile = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`http://localhost:3000/api/audio/download/${activeAudioFile._id}`, {
  ...
  const handleDeleteActiveFile = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:3000/api/audio/files/${activeAudioFile._id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const sendSingleEditToBackend = async (operationType, operationData = {}) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3000/api/audio/apply-single-edit', {
  ...
  const refreshActiveWaveform = useCallback(async (updatedFile) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`http://localhost:3000/api/audio/files/${updatedFile._id}/stream`, {
        responseType: 'blob',
        headers: {
          Authorization: `Bearer ${token}`,
        },
